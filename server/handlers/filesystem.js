...
const listDirectory = async (dirPath, recursive, prefix = '') => {
  ...
  for (const item of items) {
    ...
    if (recursive && stats.isDirectory()) {
      const childDetails = await listDirectory(itemPath, true, name);
...
export default {
  ...
  fs_list: async ({ input }) => {
    ...
    try {
      ...
      const details = await listDirectory(safe, recursive);
  ...
  fs_exists: async ({ input }) => {
    ...
    try {
      const safe = resolveSafePath(checkPath);
      const exists = await fs.pathExists(safe);
  ...
  fs_delete: async ({ input }) => {
    ...
    try {
      const safe = resolveSafePath(deletePath);
      const exists = await fs.pathExists(safe);
